...
v1.3 將「帳務來源」與「報表呈現」分開。標準結構的權責損益草案與老闆看的首屏，都必須從同一份 `POSTED` journal 生成；管理維度、分攤或重分類只能作為可追溯的管理調整，不能回頭改寫正式分錄。
...
如果執行環境可建立並驗證試算表，主要交付為 `management-dashboard.xlsx`；否則交付可閱讀的 Markdown 與可機讀的 JSON，並保留相同口徑、來源、資料信心與待辦。
