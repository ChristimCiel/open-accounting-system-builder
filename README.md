...
- 首次啟動就先顯示老闆看得懂的功能清單，而不是先問會計科目。
- AI 依已知情況預選建議；老闆可直接「照建議」、一次多選/取消，或新增自訂功能。
- AI 推薦與老闆正式確認會分開記錄；功能可後續新增、停用、改優先順序並保留版本。
- 功能可客製，但來源追溯、查重、借貸平衡、對帳、寫入前預覽與專業邊界等安全控制不能被關閉。
...
- 從單一、狀態為 `POSTED` 的複式分錄生成同口徑的權責損益與老闆首屏，不用第二套未受控的帳重算。
- 當 O6 啟用時，再讓老闆複選首屏、損益、成本、資料信心、月結與自訂管理報表。
...
先不要直接替我入帳。請先給我 AI 預選的多選功能清單，讓我可以直接照建議、一次增減多個項目，或新增自訂功能。收到我的確認後，再逐步訪談我並依已選功能建立：
...
Skill 的第一個實質問題一定是「你想讓這套系統做什麼」。它會先顯示 8 個白話結果，並根據目前已知內容預選建議：
...
你可以回覆「照建議」、「加 O3/O6、移除 O1」，或「自訂：我要看每個門市的獲利」。未收到明確選擇前，Skill 不會開始研究、讀敏感資料或建立檔案。

之後隨時輸入「修改功能」，就能新增、停用、排序或修改自訂功能；Skill 只顯示差異，老闆確認後才生效。

完成功能多選後，才進入漸進式訪談：

1. 確認功能優先順序、公司所在地、組織型態、營運階段、資料範圍與儲存位置。
...
v1.4 將「帳務來源」與「報表呈現」分開。標準結構的權責損益草案、老闆首屏與 Excel 原生圖表，都必須從同一份 `POSTED` journal 生成；管理維度、分攤或重分類只能作為可追溯的管理調整，不能回頭改寫正式分錄。

使用者仍是先複選 O1–O8；只有老闆確認 O6 後，Skill 才展開下列可再複選的報表子功能：

- `dashboard`：老闆首屏，優先回答本月賺賠、可用現金、錢卡在哪裡與今天要做什麼。
- `profit`：權責損益、每 100 元收入留下多少，以及與上期或目標的差異。
- `cost`：直接成本、營運費用、主要變動與產品／專案／通路觀點。
- `trust`：資料截止日、對帳狀態、暫估、缺憑證、衝突與待專業覆核影響。
- `close`：月結進度、阻擋項目、負責人與下一步。
- `custom`：老闆自訂門市、專案、產品或其他管理視角，由 AI 先提案，確認後才啟用。
...
- 收入、成本、費用與淨利的正負影響柱狀圖。
- 前六大成本水平長條圖。
- 成本構成甜甜圈圖：僅在所有成本非負、總額大於零且至少兩類時可啟用。
- 啟用維度後的產品、專案、門市或通路獲利柱狀圖。

如果執行環境可建立並驗證試算表，主要交付為 `management-dashboard.xlsx`，含 `Dashboard`、`P&L`、`Cost Analysis`、`Dimension Profit`、`Chart Data`、`Data & Lineage` 與 `Checks`；圖表從可見公式區取數，匯出後會重新匯入檢查原生圖表與公式錯誤。無試算表能力時，才降級為 Markdown/JSON。

儀表板必須把「本月淨利／淨損」與「現金增減／可用現金」分開呈現，不得暗示賺錢就等於已收到現金。這些都是可追溯的內部管理輸出；除非已有適切專業人士對明確範圍與日期的書面結論，不得宣稱為法定財報、已符合會計準則、已經專業覆核或可直接申報。
